defmodule LivepromptWeb.CoreComponents do
  @moduledoc """
  ...
  def simple_form(assigns) do
    ...
    <.form :let={f} for={@for} as={@as} {@rest}>
      <div class="mt-10 space-y-8 bg-neutral px-4 py-8 rounded">
  ...
  def input(%{type: "select"} = assigns) do
    ...
    <div phx-feedback-for={@name}>
      ...
      <select
        id={@id}
        name={@name}
        class="mt-2 block w-full rounded-md border border-gray-300 bg-content shadow-sm focus:border-zinc-400 focus:ring-0 sm:text-sm"
        multiple={@multiple}
        {@rest}
      >
  ...
  def input(assigns) do
    ...
    <div phx-feedback-for={@name}>
      ...
      <input
        ...
        class={[
          "input mt-2 block w-full rounded-lg focus:ring-0 sm:text-sm sm:leading-6",
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(LivepromptWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(LivepromptWeb.Gettext, "errors", msg, opts)
    end
  end
